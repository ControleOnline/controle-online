        ...
        <div class="row">
          <div class="col-12 q-mt-md">
            ...
            <ul>
              ...
              <li>No campo "Dados adicionais", coloque a seguinte instrução: Frete número #{{ order !== null ? order.id : '' }} por conta e ordem da {{ provider.name }} CNPJ: {{ provider.document }}</li>
            ...
            <div class="row">
              <div class="col-12">
                <div class="text-subtitle1 q-mt-md q-mb-md">Upload da nota fiscal</div>
...
export default {
  ...
  computed: {
    ...
    updEndpoint() {
      return `${ENTRYPOINT}${ENTRYPOINT.endsWith('/') ? '' : '/'}invoice_taxes/upload-nf?myCompany=${this.myCompany.id}`;
    },
    ...
    urlInvoiceTax() {
      ...
      return `${ENTRYPOINT}${ENTRYPOINT.endsWith('/') ? '' : '/'}vendor/pdf.js/web/viewer.html?file=/invoice_taxes/${this.invoiceTax.id}/download-nf`;        
    },
  },
    ...
    onUploadedFile(file) {
      ...
      this.onRequest()
        .then(order => {
          ...
          this.$refs.docviewer
            .setAttribute('src', this.$refs.docviewer.getAttribute('src'));
        });
          ...
          if (order !== null) {
            if (order.carrier.address == null || order.carrier.document == null)
              ...
              if (order.invoiceTax.length > 0) {
                let invoiceTax  = order.invoiceTax.find(inv => inv.invoice_type == 55);
