      ...
      <template v-slot:body="props">
        <q-tr :props="props">
          <q-td key="id"             :props="props">
            <q-btn outline dense
              :to   ="{ name: 'ComissionDetails', params: { id: props.row.id } }"
          ...
          <q-td key="pedidos"        :props="props">
            <q-btn outline dense
              ...
              color ="primary"
              @click="seeOrdersList(props.row.pedidos, props.row.main_id)"
              class ="full-width"
            />
          </q-td>
...
export default {
  ...
  computed: {
    ...mapGetters({
      isLoading : 'comissionInvoice/isLoading' ,
      error     : 'comissionInvoice/error'     ,
      violations: 'comissionInvoice/violations',
      items     : 'comissionInvoice/items'     ,
      totalItems: 'comissionInvoice/totalItems',
    ...
    items(items) {
      ...
      for (let i in items) {
        ...
        for (let o in item.order) {          
          orders.push(
            item.order[o].order.mainOrder['@id'].match(/^\/sales\/orders\/([a-z0-9-]*)$/)[1]
          );
        }

        data.push({
          '@id'           : item['@id'],
          'id'            : item['@id'].match(/^\/finance\/comission\/([a-z0-9-]*)$/)[1],
          'pedidos'       : orders,
  ...
  methods: {
    ...mapActions({
      getItems   : 'comissionInvoice/getItems'   ,
      reset      : 'comissionInvoice/reset'      ,
      getStatuses: 'comissionInvoice/getStatuses',
    }),
