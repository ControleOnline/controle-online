        ...
        >
          <tbody>
            <tr>
              ...
              <td class="text-left">
                {{ `#${this.invoice['@id'].match(/^\/finance\/comission\/([a-z0-9-]*)$/)[1]}` }}
              </td>
            </tr>
            <tr>
...
export default {
  ...
  methods: {
    ...mapActions({
      getInvoice: 'comissionInvoice/getInvoice',
    }),
    ...
    requestInvoice(invoiceId) {
      ...
      params['myProvider'] = this.myCompany.id;
        ...
        .then(data => {
          ...
          if (data['@id']) {
            ...
            if (this.invoice.order.length > 0) {              
              this.orderId     = this.invoice.order[0].order['@id'].match(/^\/comission\/orders\/([a-z0-9-]*)$/)[1];              
