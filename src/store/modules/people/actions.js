...
export function loadPeople (context, peopleId) {
  fetch(`/people/${peopleId}`, { method: 'GET' })
    .then(response => response.json())
    .then(response => {
      context.commit('SET_PEOPLE', response.response.data)
    })
}
...
export const getClientContact = ({ commit }, searchBy) => {
  return fetch(`${RESOURCE_ENDPOINT}/client-company`, { params: searchBy })
    .then(response => response.json())
    .then(data => {
      if (data.response) {
...
export const getMyCompanies = ({ commit }) => {
  return fetch(`${RESOURCE_ENDPOINT}/my-companies`)
    .then(response => response.json())
    .then(data => {
      return data.response;
