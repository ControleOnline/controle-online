...
export const bankItau = ({ commit }, { invoiceId, operation, params = {} }) => {
  return fetch(`/finance/receive/${invoiceId}/bank/itau/${operation}`, { params })
    .then(response => response.json())
    .then(data => {
      if (data.response) { return data.response.data !== null ? data.response.data : null }

      return null
    })
}

export const getOrderClasses = ({ commit }, { invoiceId, params = {} }) => {
  return fetch(`/finance/receive/${invoiceId}/order-classes`, { params })
    .then(response => response.json())
    .then(data => {
